...
class typedtuple(object):
    ...
    _one_arg_needed = "typedtuple needs one argument that will serve as the structname.{}".format(
        _example_usage)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields = kwargs
        self.name = args[0]
        self.namedtuple = namedtuple(self.name, self.fields.keys())
    ...
    You might want to try:
        {k:} = {type_name:}\n\n""".format(
                k=key,
                type_name=type(kwargs[key]).__name__,
                val_repr=kwargs[key].__repr__())
    ...
    def validate_fields(self, **kwargs):
        ...
        for field in self.fields:
            ...
            if type(value) != required_type:
                raise IllegalTypedTuple(
                    '{}.{} needs to be a {}, recieved: {}({})'.format(
                        self.name, field, required_type.__name__,
                        type(value).__name__, value.__repr__()))
...
"""
    ...
    def __init__(self, *args, **kwargs):
        args = args
        kwargs = kwargs
        self.namedtuple = namedtuple(args[0], kwargs.keys())
        self.name = args[0]
            ...
            for r in self.__saved_rules__[k]:
                if not r(v):
                    raise IllegalStrictTuple(
                        '\n------------------------------------\ninstruction:\n\t{}.{} = {}({})\nviolates the rule:\n\t{}\n------------------------------------'.
                        format(self.name, k,
                               type(v).__name__, repr(v), getsource(r)
                               .strip()))
...
if __name__ == '__main__':
    ...
    Point = typedtuple('Point', x=int, y=int)
    ...
    Point = stricttuple(
        "Point",
        x=(lambda x: type(x) == int , lambda x: x < 150 and x >= 0),
        y=(lambda y: type(y) == int , lambda y: y < 150 and y >= 0))

    from random import randint
    for i in range(32):
        t = Point(x=randint(0, 150), y=randint(0, 150))
        print(repr(t))
